import EzTrackTitle from "../components/EzTrackTitle.jsx";
import {useState} from "react";
import {useNavigate, useSearchParams} from "react-router-dom";
import {useApiClient} from "../services/apiClient.js";


export default function ValidateRecovery() {

  const [searchParams, setSearchParams ] = useSearchParams();
  const origEmail = searchParams.get("email");
  const [email, setEmail] = useState(origEmail ?? "");
  const [validationCode, setValidationCode] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [password, setPassword] = useState("");
  const {completePasswordRecovery} = useApiClient();
  const navigate = useNavigate();

  const validateRecovery = () => {
    completePasswordRecovery({password: password, email: email, validationCode: validationCode})
    .then(() => {
      navigate("/login");
    }).catch(error => {
      console.log(error)
    });
  }

  return <>
    <EzTrackTitle pageName={"Validate Password Recovery"} showLogout={false} />
    <div><hr/></div>
    <div style={{ maxWidth: 420, margin: "2rem auto", padding: "2rem" }}>
      <h2 className="signupCaption" style={{ marginBottom: 4 }}>Reset Password</h2>
      <p style={{  marginBottom: 24 , color: "#34c6bd", fontSize:20}}>Please enter your email, validation code and, new password.</p>
      <label style={{  color: "#34c6bd", display: "block", fontSize: 20, fontWeight: 500, marginBottom: 6, textTransform: "capitalize" }}>
        Email:
      </label>
      <input
        style={{ fontSize:20, width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #d1d5db", boxSizing: "border-box",  }}
        disabled={origEmail !== null} type={"text"} name={"email"} value={email}
        onChange={(e) => setEmail(e.target.value)}/>
      <label style={{  marginTop:"2rem", color: "#34c6bd", display: "block", fontSize: 20, fontWeight: 500, marginBottom: 6, textTransform: "capitalize" }}>
        Validation Code:
      </label>
      <input
        style={{ fontSize:20, width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #d1d5db", boxSizing: "border-box",  }}
        type={"text"} name={"validation-code"} value={validationCode} onChange={(e) => setValidationCode(e.target.value)}/>
      <label style={{marginTop:"2rem", color: "#34c6bd", display: "block", fontSize: 20, fontWeight: 500, marginBottom: 6 }}>Password</label>
      <div style={{ position: "relative" }}>
        <input
          type={showPassword ? "text" : "password"}
          name="password"
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          placeholder="Min. 8 characters"
          style={{ fontSize:20, width: "100%", padding: "8px 40px 8px 12px", borderRadius: 8, border: "1px solid #d1d5db", boxSizing: "border-box" }}
        />
        <button
          onClick={() => setShowPassword((v) => !v)}
          style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#6b7280" }}
          aria-label={showPassword ? "Hide password" : "Show password"}
        >
          {showPassword ? "🙈" : "👁️"}
        </button>
      </div>
      <button style={{marginTop:"2rem"}} onClick={validateRecovery}>Validate</button>
    </div>
  </>
}